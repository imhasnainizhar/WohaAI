...
pub fn init_env() {
    ...
    if !is_production {
        ...
        match from_path(env_path) {
            Ok(_) => info!("Loaded environment from: {}", env_path.display()),
            Err(err) => warn!("Did not load env file '{}': {}", env_path.display(), err),
        }
    } else {
...
impl Env {
    pub fn load() -> Self {
        Self {
            ...
            fluvio_api_uri: env::var("AUTH_FLUVIO_API_URI").expect("AUTH_FLUVIO_API_URI env var not set"),
            FLUVIO_SC: env::var("AUTH_FLUVIO_SC").expect("AUTH_FLUVIO_SC env var not set"),
        }
    }
}
