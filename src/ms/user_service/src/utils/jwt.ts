...
export function createValidationToken(sessionId: string, step: string) {
  return jwt.sign(
...
export function verifyValidationToken(token: string, expectedStep: string): { sid: string, step: string } {
  try {
