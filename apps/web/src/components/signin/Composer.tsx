...
export default function SigninComposer() {
    ...
    const steps = [
        {
            ...
            render: (next: (v: any) => void) => (
                <SigninUsernameOrEmail next={next} setNextStep={setNextStep} data={dataForm} />
            ),
        },
        {
            name: "Password",
            render: (next: (v: any) => void) => (
                <SigninPassword next={next} />
            )
        },
    ];

    const next = (values: any) => {
        const merged = { ...dataForm, ...values };
        setDirection(1);
        setForm(merged);
        if (nextStep === null) {
