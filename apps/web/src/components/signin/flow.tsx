...
export default function SigninFlow() {
    ...
    const dataForm = form as SigninForm;
    ...
    const steps = [
        {
            ...
            render: (next: (v: any) => void) => (
                <SigninUsernameOrEmail next={next} setNextStep={setNextStep} data={form} />
            ),
        },
        {
            name: nextStep === "email" ? "Email" : "Password",
            render: (next: (v: any) => void) =>
                nextStep === "email" ? (
                    <SigninEmail next={next} setNextStep={setNextStep} data={{ username: dataForm.username }} />
                ) : (
                    <SigninPassword next={next} data={dataForm} />
                ),
        },
    ];
    ...
    const next = (values: any) => {
        ...
        setForm(merged);
        if (nextStep === null) {
            console.log("Internal Server Error, Signin step null");
            return null;
        }
