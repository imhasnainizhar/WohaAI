...
import { ApiResponseOptions } from '../../../../../../packages/http/dist/response';

export interface AuthNextStepResponse {
    nextStep: "email" | "username" | "password";
}

export default function SignupInit({ next, setNextStep, data }: { next: (next: any) => void, setNextStep: any, data?: any }) {
    const {
        register,
        handleSubmit,
        reset,
        formState: { errors },
    } = useForm<{ usernameOrEmail: string }, any, SignupInitRequest>({
        resolver: zodResolver(SignupInitRequestSchema),
    });
    ...
    const onSubmit = async (values: SignupInitRequest) => {
        setSignInError("");
    ...
    const onGetStarted = async (reqData: SignupInitRequest, next: any) => {
        setSignInError("");
